...
This project can be run either using Docker for a streamlined, one-command setup, or manually with Python.

### Option 1: Run with Docker (Recommended)

This method simplifies setup by running the entire pipeline within a containerized environment.
...
1.  **Build the Docker image:**
    ...
    ```bash
    docker build -t pharma-data-pipeline .
    ```

2.  **Run the container:**
    This command will execute the entire data collection, validation, and report generation pipeline. The output files (database, logs, and reports) will be saved to your local `data/` and `logs/` directories.
    ```bash
    docker run --rm --name pharma-pipeline \
      -v $(pwd)/data:/app/data \
      -v $(pwd)/logs:/app/logs \
      -v $(pwd)/data_quality_report.md:/app/data_quality_report.md \
      pharma-data-pipeline
    ```
    *Note for Windows users: Replace `$(pwd)` with `${PWD}` if using PowerShell.*

#### Expected Outcome
The container will run all three scripts sequentially (`collect_drug_data.py`, `validate_data.py`, `generate_report.py`). After execution, the following files will be created/updated in your local project directory:
-   `data/drugs.db`
...
Follow these steps to run the pipeline directly on your machine.
...
This generates `data_quality_report.json` with comprehensive metrics. You can then run `python generate_report.py` to create the markdown summary.
